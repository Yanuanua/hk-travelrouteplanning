    ...
    while len(route) < total_spots:
        ...
        if all(theme in visited_themes for theme in themes):
            ...
            if total_time + next_spot_info['time'] + traffic_m1[current_spot][next_spot] <= max_time + 20:
                ...
                remaining_spots = [spot for spot in remaining_spots if spots[spot]["time"] + total_time + traffic_m1[current_spot][spot] <= max_time + 20]
                if not remaining_spots:
                    break
                next_spot = max(remaining_spots, key=lambda x: tratio*(postpre*tran_post[route[-1]][x]+(1-postpre)*tran_pre[route[-1]][x])+sratio*(postpre*spots[x]['spot_post']+(1-postpre)*spots[x]['spot_pre']))
                next_spot_info = spots[next_spot]
...
def route_method2(start_point, spots_num, themes, themes_num, postpre):
    ...
    if spots_num > themes_num:
        ...
        combined_df = combined_df[~combined_df['themes'].apply(lambda x: any(theme in x for theme in themes))]
...
st.write('Suitable for tourists who follow popular attractions but have limited time, are in the attractions, or have certain intentions.')
